...
;;;; Compilateur LISP vers assembleur MIPS (Version Bootstrap - Phase 10)
;;;; Cette version utilise des primitives LISP pures pour permettre l'auto-compilation

;; Note: loader.lisp est chargé par main.lisp avant compiler.lisp

;;; ============================================================================
;;; CHARGEMENT DES PRIMITIVES BOOTSTRAP
;;; ============================================================================

;; Charger les primitives réécrites en LISP pur
...
(defun lookup-function (env fn-name)
  ...
  (cdr (my-assoc fn-name (compiler-env-functions env))))
...
qui a défini la fonction, ou NIL si non trouvée."  
  (let ((current-env env))
    (loop
      ...
      (let ((label (cdr (my-assoc fn-name (compiler-env-functions current-env)))))
        (when label
...
(defun alloc-temp-register (env)
  ...
  (let ((reg-num (mod (compiler-env-register-counter env) 10)))
    ...
    (intern (my-format-register reg-num))))
...
(defun lookup-variable-with-depth (env var)
  ...
  (let ((current-env env))
    (loop
      ...
      (let ((location (cdr (my-assoc var (compiler-env-variables current-env)))))
        (when location
         ...
         (labels
          (if (and (>= (length args) 2)
              ...
              (let* ((definitions (first args))
                     ...
                     (func-names (my-map-first definitions))
  ...
  (let* ((new-env (make-lexical-env env nil))  ; Créer scope SANS incrémenter (LABELS body n'a pas de frame)
    ...
    (dolist (def definitions)
      (let* ((fn-name (first def))
             (fn-args (second def))
             (fn-label (gen-label new-env (my-format-label "LOCAL" fn-name))))
    ...
    (dolist (def definitions)
      (let* ((fn-name (first def))
             (fn-args (second def))
             (fn-body (cddr def))
             (fn-info (cdr (my-assoc fn-name fn-infos)))
...
(defun compile-and-run (expr &key (verbose nil))
  ...
  (let ((vm (make-new-vm :verbose verbose))
    ...
    ;; Debug output disabled in bootstrap version
...
;; Bootstrap version - startup messages disabled
