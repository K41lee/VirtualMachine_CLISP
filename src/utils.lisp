...
(defun dump-vm-state (vm)
  ...
  (dump-stack vm 5)
  (format t "~%=== ZONE TAS ===~%")
  (let ((gp (get-register vm :$gp)))
    (format t "Global pointer ($gp): ~A~%" gp)
    (when (> gp +heap-start+)
      (dump-memory vm +heap-start+ (min (1- gp) (+ +heap-start+ 20)))))
  (format t "~%")
  (format t "================================================================================~%"))
...
(defun get-vm-stats (vm)
  ...
  (let ((sp (get-register vm :$sp))
        ...
        (gp (get-register vm :$gp))
        (stack-max (- *maxmem* *code-size*)))
    (list :instructions (vm-instruction-count vm)
          :state (vm-state vm)
          :heap-used (- gp +heap-start+)
          :heap-available (- (get-register vm :$sp) gp)
